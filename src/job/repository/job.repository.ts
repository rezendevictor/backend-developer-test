...
export class JobRepository {
  ...
  public async putArchiveJobPosting(id: string) {
    return this.updateQueryByIdAndStatus(
      JobStatus.ARCHIVED,
      id,
      JobStatus.PUBLISHED,
    );
  ...
  public async updateQueryByIdAndStatus(
    statusToSet: JobStatus,
    id: string,
    statusToSearch: JobStatus,
  ) {
    const updateQuery = `
        UPDATE jobs 
        SET status = $1 
  ...
  async deleteJobByStatusAndId(id: string): Promise<JobEntity> {
    const deleteQuery = `
        ...
        RETURNING *;`;
    const result: QueryResult<JobEntity> = await pool.query(deleteQuery, [
      id,
      JobStatus.DRAFT,
    ]);
    return result.rows[0];
