...
describe("CompanyContoller", () => {
  ...
  it("Should return one company by id", async () => {
    ...
    jest.spyOn(companyService, "getCompanyById").mockImplementation(() => {
      return Promise.resolve(companyDTO);
    });

    await companyController.getCompanyById(randomUUID(), {
    ...
    expect(companyService.getCompanyById).toBeCalledTimes(1);
  });
  ...
  it("Should return list of companies by id", async () => {
    ...
    jest.spyOn(companyService, "getCompanies").mockImplementation(() => {
    ...
    expect(jsonMethod.mock.calls[0][0]).toMatchObject({
      companies: [companyDTO, companyDTO2],
    });

    expect(companyService.getCompanies).toBeCalledTimes(1);
  });
});
