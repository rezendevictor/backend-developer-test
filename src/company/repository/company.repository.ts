...
export class CompanyRepository {
    public async getCompanies(): Promise<any> {
        const result : QueryResult<CompanyDTO[]> = await pool.query('SELECT * FROM companies');
        return result.rows;
    }

    public async getCompanyById(id:string): Promise<any>{
        const result : QueryResult<CompanyDTO[]> = await pool.query('SELECT * FROM companies WHERE id = $1 limit 1', [id]);
